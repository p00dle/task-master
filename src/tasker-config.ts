...
> {
  ...
  public session<N extends string, T, PS extends keyof ST, C extends CN>(
    name: N,
    dependencies: SessionDepsOptions<PS, C>,
    options: SessionOptions<
      T &
        (PS extends keyof ST ? { parentSession: SessionObject<ST[PS]> } : never) &
        (C extends CN ? { validateCredentials: (valid: boolean) => any } : never)
    >
  ) {
