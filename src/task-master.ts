...
type StatusListener = (data: any) => any;
...
const DEFAULT_CONFIG = {
  ...
  logs: {
    ...
    dumpLogsOnExitToFilename: undefined,
  },
};

function normalizeConfig(config: TaskMasterConfig) {
  return {
    forceStartTasks: config.autostartTasks || DEFAULT_CONFIG.autostartTasks,
    startGui: !!config.gui ? true : false,
    guiPort: (config.gui && config.gui.port) || DEFAULT_CONFIG.gui.port,
    openInBrowser: (config.gui && config.gui.openInBrowser) || DEFAULT_CONFIG.gui.openInBrowser,
    localConnectionsOnly:
      config.gui && typeof config.gui.localConnectionsOnly !== 'undefined'
        ? config.gui.localConnectionsOnly
        : DEFAULT_CONFIG.gui.localConnectionsOnly,
    longPollTimeout: (config.gui && config.gui.longPollTimeout) || DEFAULT_CONFIG.gui.longPollTimeout,
    shouldLog: typeof config.logs === 'boolean' ? config.logs : true,
    logsFilename: (config.logs && config.logs.filename) || DEFAULT_CONFIG.logs.filename,
    logConsole: (config.logs && config.logs.console) || DEFAULT_CONFIG.logs.console,
    logStore: (config.logs && config.logs.store) || DEFAULT_CONFIG.logs.store,
    logHttpRequests: (config.logs && config.logs.logHttpRequests) || DEFAULT_CONFIG.logs.logHttpRequests,
    logLevel: (config.logs && config.logs.level) || DEFAULT_CONFIG.logs.level,
    logConsumer: (config.logs && config.logs.consumer) || DEFAULT_CONFIG.logs.consumer,
    logMemoryLimitMb: (config.logs && config.logs.memoryLimitMb) || DEFAULT_CONFIG.logs.memoryLimitMb,
    logMemoryPurgeRatio: (config.logs && config.logs.memoryPurgeRatio) || DEFAULT_CONFIG.logs.memoryPurgeRatio,
    useArchive: typeof config.logs === 'boolean' ? config.logs : DEFAULT_CONFIG.logs.useArchive,
    archiveLogsAfterMs: (config.logs && config.logs.archiveAfterMs) || DEFAULT_CONFIG.logs.archiveAfterMs,
    retainArchivedLogs: (config.logs && config.logs.retainArchivedLogs) || DEFAULT_CONFIG.logs.retainArchivedLogs,
    archiveLogsIntervalMs: (config.logs && config.logs.archiveIntervalMs) || DEFAULT_CONFIG.logs.archiveIntervalMs,
    dumpLogsOnExitToFilename:
      (config.logs && config.logs.dumpLogsOnExitToFilename) || DEFAULT_CONFIG.logs.dumpLogsOnExitToFilename,
  };
}
type StatusType = 'credentials' | 'sessions' | 'dataSources' | 'tasks';
...
> {
  ...
  protected dumpLogsOnExitToFilename: string | null = null;
